...
// Definimos la tabla temporal para redb. 
const TEMP_TABLE: TableDefinition<&str, &[u8]> = TableDefinition::new("temp_table");

#[derive(Clone)]
struct AppState {
    db_reader: DbReader,
    db_writer_tx: mpsc::Sender<DbMessage>,
...
const RECORD_SIZE: u64 = (DATA_SIZE + HEADER_SIZE) as u64; 
const DB_CAPACITY_BYTES: u64 = 4 * 1024 * 1024 * 1024;     
const MAX_RECORDS: u32 = (DB_CAPACITY_BYTES / RECORD_SIZE) as u32;

// --- TIPOS DE MENSAJES PARA EL ACTOR ---
pub enum DbMessage {
    Append {
        data: Box<[u8; DATA_SIZE]>, 
        responder: oneshot::Sender<io::Result<u32>>,
    },
    Overwrite {
        index: u32,
        data: Box<[u8; DATA_SIZE]>,
...
impl DbReader {
    pub fn read_raw(&self, index: u32) -> io::Result<[u8; DATA_SIZE]> {
        if index >= MAX_RECORDS {
            return Err(io::Error::new(io::ErrorKind::InvalidInput, "Índice fuera de rango"));
        }

        let offset = (index as u64) * RECORD_SIZE;
        let mut raw_buffer = [0u8; RECORD_SIZE as usize];
        
        self.file.read_exact_at(&mut raw_buffer, offset)?;

        let mut data = [0u8; DATA_SIZE];
        data.copy_from_slice(&raw_buffer[1..]);
        Ok(data)
    }
}

// --- EL NÚCLEO DEL ACTOR ---
struct CircularDbCore {
    file: Arc<File>,
    cursor: u32,
    phase: u8,
}
...
// --- BUCLE DEL ACTOR ASÍNCRONO ---
pub async fn start_db_actor(path: &str) -> io::Result<(mpsc::Sender<DbMessage>, DbReader)> {
    let (mut core, reader) = CircularDbCore::open(path)?;
    let (tx, mut rx) = mpsc::channel::<DbMessage>(1000);

    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            match msg {
                DbMessage::Append { data, responder } => {
                    let result = core.append(&data);
                    let _ = responder.send(result);
                }
                DbMessage::Overwrite { index, data, responder } => {
                    let result = core.overwrite(index, &data);
                    let _ = responder.send(result);
                }
            }
        }
        println!("Actor de base de datos detenido.");
    });

    Ok((tx, reader))
}

use blake2::digest::{KeyInit, Mac, Digest}; // <-- Importante: Añadido Digest para el hash libre
...
impl Cell {
    pub fn from_bytes(bytes: &[u8; DATA_SIZE]) -> Self {
        ...
        Self { salt, challenge, index_l, index_r, cmd, padding }
    }

    pub fn to_bytes(&self) -> Box<[u8; DATA_SIZE]> {
        let mut bytes = Box::new([0u8; DATA_SIZE]);
...
async fn serve_index() -> impl axum::response::IntoResponse {
    // Lee el archivo directamente del sistema de archivos en cada petición
    match fs::read_to_string("index.html").await {
        Ok(contenido) => {
            // Retorna el HTML con código 200
            Html(contenido).into_response()
        }
        Err(err) => {
            // Si el archivo no existe o no se puede leer, devuelve un error 500
            eprintln!("Error al leer index.html: {}", err);
            (
                axum::http::StatusCode::INTERNAL_SERVER_ERROR,
                "No se encontró el archivo index.html en el servidor."
            ).into_response()
        }
    }
}
async fn handle_api_sign(
    State(state): State<AppState>,
    body: Bytes,
) -> Result<Bytes, StatusCode> {
    if body.len() < 96 { return Err(StatusCode::BAD_REQUEST); }

    let firma_bytes = &body[0..64];
    let user_pub_bytes = &body[64..96];
    let payload_data = &body[96..];

    let user_pub_key = VerifyingKey::from_bytes(user_pub_bytes.try_into().unwrap())
        .map_err(|_| StatusCode::BAD_REQUEST)?;
    let signature = Signature::from_slice(firma_bytes).map_err(|_| StatusCode::BAD_REQUEST)?;

    let mensaje_firmado = &body[64..]; 
    if user_pub_key.verify(mensaje_firmado, &signature).is_err() {
        return Err(StatusCode::UNAUTHORIZED);
    }

    let mut cert_bytes = Vec::with_capacity(40 + payload_data.len());
    cert_bytes.extend_from_slice(user_pub_bytes);
    
    let exp_timestamp = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs() + 86400;
    cert_bytes.extend_from_slice(&exp_timestamp.to_le_bytes());
    cert_bytes.extend_from_slice(payload_data);

    let firma_servidor = state.server_keypair.sign(&cert_bytes);
    ...
    Ok(Bytes::from(respuesta))
}

async fn handle_api_verify(
    State(state): State<AppState>,
    body: Bytes,
) -> Result<Bytes, StatusCode> {
    if body.len() < 130 { return Err(StatusCode::BAD_REQUEST); }
    ...
    if body.len() < 130 + tamano_cert { return Err(StatusCode::BAD_REQUEST); }

    let certificado_bytes = &body[130 .. 130 + tamano_cert];
    let peticion_bytes    = &body[130 + tamano_cert ..];

    let sig_servidor = Signature::from_slice(firma_servidor_bytes).map_err(|_| StatusCode::UNAUTHORIZED)?;
    if state.server_keypair.verifying_key().verify(certificado_bytes, &sig_servidor).is_err() {
        return Err(StatusCode::UNAUTHORIZED);
    }
    ...
    if ahora > expiracion { return Err(StatusCode::UNAUTHORIZED); }

    let user_pub_key = VerifyingKey::from_bytes(&user_pub_key_bytes).unwrap();
    let sig_peticion = Signature::from_slice(firma_peticion_bytes).map_err(|_| StatusCode::UNAUTHORIZED)?;
    if user_pub_key.verify(peticion_bytes, &sig_peticion).is_err() {
        return Err(StatusCode::UNAUTHORIZED);
    }
...
async fn handle_public_get(
    ...
    let mut key_str = req_uri.path().trim_start_matches('/');
    if key_str.is_empty() {
        key_str = "/"; 
    }

    let read_txn = state.temp_db.begin_read().map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    let table = read_txn.open_table(TEMP_TABLE).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;

    if let Some(entry) = table.get(key_str).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)? {
        let value_bytes = entry.value();
        if value_bytes.len() < 4 { return Err(StatusCode::INTERNAL_SERVER_ERROR); }
        
        let owner_index = u32::from_le_bytes(value_bytes[0..4].try_into().unwrap());
        let data_real = &value_bytes[4..];

        if let Ok((cell, _)) = ciclidb_read_libre(&state.db_reader, owner_index) {
            if (cell.cmd & CMD_LEER_LIBRE) != 0 {
                let html_string = String::from_utf8_lossy(data_real).to_string();
                return Ok(Html(html_string));
            } else {
...
async fn handle_root(
    ...
    // Si nos visitan desde api.diarsaba.com (o localhost si haces pruebas con alias)
    if hostname.starts_with("api.") {
        Ok(serve_index().await.into_response())
    } else {
        // Si es diarsaba.com, delegamos a tu función existente de base de datos.
        // handle_public_get ya convierte la ruta "/" a la llave "index" automáticamente.
        match handle_public_get(state, req_uri).await {
...
#[tokio::main]
async fn main() {
    println!("Iniciando Motor CicliDB...");

    let (db_writer_tx, db_reader) = start_db_actor("ciclidb_produccion.db")
        .await
        .expect("Error fatal: No se pudo iniciar el disco circular");

    let temp_db = Database::create("tempdb.redb").expect("Error al crear tempdb");
    let write_txn = temp_db.begin_write().unwrap();
    write_txn.open_table(TEMP_TABLE).unwrap();
    write_txn.commit().unwrap();

    let server_keypair = SigningKey::from_bytes(&[1u8; 32]);

    inyectar_genesis_si_vacio(&db_reader, &db_writer_tx).await;

    let app_state = AppState {
        db_reader,
        db_writer_tx,
        temp_db: Arc::new(temp_db),
        server_keypair,
    };

    let app = Router::new()
        .route("/", get(handle_root))
        .route("/sign", post(handle_api_sign).get(handle_public_get))
        .route("/verify", post(handle_api_verify).get(handle_public_get))
        .route("/do", post(handle_api_do).get(handle_public_get))
        .fallback(handle_public_get) 
        .with_state(app_state);

    let listener = tokio::net::TcpListener::bind("0.0.0.0:3000").await.unwrap();
    println!("🚀 Servidor en línea en http://0.0.0.0:3000");
